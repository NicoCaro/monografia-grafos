...
manera análoga se escribe $e \in G$ para una arista de $G$.
...
\caption{En \subref{fig:ej0} se muestran los grafos disjuntos $G$ y $G'$. El resultado de $G * G'$ se observa en \subref{fig:join}.
Finalmente en \subref{fig:grilla} se muestra $G \ \square \
G'$.}
\label{fig:ej}
\end{figure}
...
\begin{ex}[morfismos entre caminos]
	...
	$ V = \left \lbrace {v_1 , \ldots , v_n} \right \rbrace $ donde $ v_i
	\neq v_j, \ \forall i \neq j $. Se tiene además $e_i = v_{i}v_{i+1}$
	para $ E = \left \lbrace {e_1, \ldots, e_{n-1}  } \right \rbrace$.
	Los grafos del tipo $ P_n $ se denotan como \textbf{caminos} de largo
	$n$. En el caso $n=1$ se considera $E = \emptyset$.
...
$P_n$ y $P_{n-1}$ para $n \geq 2$.

\end{ex}
\begin{solution}
	Para el caso $n = 2$ no es posible obtener un morfismo, pues $ P_1 $
	no tiene aristas. Para $ n \geq 3 $, se definen los caminos $ P_n $
	con vertices $ V = \left \lbrace {v_1 , \ldots , v_n} \right \rbrace $ y $ P_{n-1} $ con vértices $ V' = \left \lbrace {v_1 , \ldots , v_{n-1} } \right \rbrace $, cada arista de $ P_n $ se denota por $e_i$ para $ i = 1, \ldots, n-1$. De manera análoga (hasta $n-2$) se denotan las aristas de $P_{n-1}$.

	En este caso es posible construir un morfismo $f: V \rightarrow V'$
	por medio de :
	\begin{equation*}
		f(v_i) = \begin{cases}
								v_i, \ \ \ \ \ \text{para} \ i=1, \ldots, n-1 \\
								v_{n-2}, \ \text{para} \ i = n
						 \end{cases}
	\end{equation*}

	Por medio de $f$, para cada arista $e_i = v_i v_{i+1}$ en $E(P_n)$
	existe su arista idéntica $f(v_i)f(v_{i+1}) = e_i$ en $E(P_{n-1})$ para $i = 1, \ldots, n-2$. Finalmente para la arista $e_{n-1} = v_{n-1} v_{n}$  existe $f(v_{n-1}) (v_{n}) = v_{n-1} v_{n-2} = v_{n-2} v_{n-1} \in E(P_{n-1})$. Por lo que se confirma que $f$ es
